...
function config_url {
    echo "https://dl.cloudsmith.io/public/symfony/stable/config.deb.txt?distro=${distro}&codename=${codename}&version=${version}&arch=${arch}" | sed 's/ /%20/g'
}
...
function import_gpg_key {
    local text="Importing 'symfony/stable' repository GPG key ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/symfony-stable-archive-keyring.gpg"
    curl -1sLf "https://dl.cloudsmith.io/public/symfony/stable/gpg.99CEF8A62216B73D.key" | gpg --dearmor > $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/symfony-stable.gpg
    }
...
function setup_repository {
    local repo_path="/etc/apt/sources.list.d/symfony-stable.list"
    check_fetch_config

    local text="Installing 'symfony/stable' repository via apt ..."
    echo_running "$text"
